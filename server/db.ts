...
export function initTables(): void {
  ...
  try { d.exec("ALTER TABLE goals ADD COLUMN start_value REAL DEFAULT 0;"); } catch (e: any) { console.warn("[DB] Migration start_value:", e.message); }
  
  // Migrate: add is_starred and is_archived to notes if missing
  try { d.exec("ALTER TABLE notes ADD COLUMN is_starred INTEGER DEFAULT 0;"); } catch (e: any) { console.warn("[DB] Migration is_starred:", e.message); }
  try { d.exec("ALTER TABLE notes ADD COLUMN is_archived INTEGER DEFAULT 0;"); } catch (e: any) { console.warn("[DB] Migration is_archived:", e.message); }

  // Migrate: add is_all_day to tasks if missing
  try { d.exec("ALTER TABLE tasks ADD COLUMN is_all_day INTEGER DEFAULT 0;"); } catch (e: any) { console.warn("[DB] Migration is_all_day:", e.message); }

  // Migrate: add due_end_date to tasks if missing
  try { d.exec("ALTER TABLE tasks ADD COLUMN due_end_date TEXT;"); } catch (e: any) { console.warn("[DB] Migration due_end_date:", e.message); }
}
...
export function clearSession(): void {
  ...
  sessionKey = null;
  sessionToken = null;
  if (db) { try { db.close(); } catch (e: any) { console.warn("[DB] Error closing DB:", e.message); } db = null; }
}
...
export function secureDelete(filePath: string): void {
  ...
  try {
    ...
    try { fs.unlinkSync(filePath); } catch (err: any) { console.warn(`[FactoryReset] Failed to unlink ${filePath}:`, err.message); }
  }
}
...
export function tryDeleteFiles(): void {
  ...
  for (const f of coreFiles) {
    secureDelete(f);
    try { fs.unlinkSync(f); } catch (err: any) { console.warn(`[tryDeleteFiles] Failed to unlink ${f}:`, err.message); }
  }
  // Wipe backups
  if (fs.existsSync(BACKUP_DIR)) {
    for (const f of fs.readdirSync(BACKUP_DIR)) {
      ...
      secureDelete(fp);
      try { fs.unlinkSync(fp); } catch (err: any) { console.warn(`[tryDeleteFiles] Failed to unlink ${fp}:`, err.message); }
    }
  }
}
...
export async function performFactoryReset(): Promise<void> {
  ...
  try { await deleteAutologinKey(); } catch (err: any) { console.warn("[FactoryReset] Error deleting autologin key:", err.message); }
