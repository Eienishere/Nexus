...
import { isDbOpen, getSessionToken } from "./db.js";
...
export function recordFailedAttempt(ip: string): { locked: boolean; retryAfterSeconds: number } {
  // Prevent RAM exhaustion DoS (simple LRU-like behavior)
  if (attemptMap.size > 10000 && !attemptMap.has(ip)) {
    const firstKey = attemptMap.keys().next().value;
    if (firstKey) attemptMap.delete(firstKey);
  }
...
/** Middleware that rejects requests when DB is locked or token is invalid. */
