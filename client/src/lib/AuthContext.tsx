...
export function AuthProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<AuthState>({
    ...
    token: localStorage.getItem("nexus-token") || null,
  });

  // Patch window.fetch to automatically include the token for /api/ requests
  useEffect(() => {
  ...
  const setup = useCallback(async (password: string) => {
    try {
      ...
      setState(prev => ({ ...prev, isFirstRun: false, isLocked: false, failedAttempts: 0, token: data.token }));
      return { success: true };
    } catch {
  ...
  const unlock = useCallback(async (password: string) => {
    try {
      ...
      if (data.token) localStorage.setItem("nexus-token", data.token);
      setState(prev => ({ ...prev, isLocked: false, failedAttempts: 0, retryAfterSeconds: 0, token: data.token }));
      return { success: true };
    } catch {
      return { success: false, error: "Sunucuya bağlanılamadı." };
    }
  ...
  const lock = useCallback(async () => {
    ...
    setState(prev => ({ ...prev, isLocked: true, failedAttempts: 0, retryAfterSeconds: 0, token: null }));
  }, []);
