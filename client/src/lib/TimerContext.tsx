...
export function TimerProvider({ children }: { children: React.ReactNode }) {
  ...
  const alarmTickFnRef = useRef<(() => void) | null>(null);

  // Local clock PiP için saniye güncelleme state'i
  const [clockSecond, setClockSecond] = useState(0);

  useEffect(() => {
    // Request notification permission globally on mount
  ...
  // Resilient Web Worker callback for Clock Alarms (prevents background throttling issue)
  const lastCheckedMinuteRef = useRef<string>(format(new Date(), 'HH:mm'));

  useEffect(() => {
    alarmTickFnRef.current = () => {
      const now = new Date();
      const currentHm = format(now, 'HH:mm');
      if (currentHm !== lastCheckedMinuteRef.current) {
        lastCheckedMinuteRef.current = currentHm;
  ...
  const triggerAlarm = async (type: 'pomodoro' | 'countdown' | 'alarm', message: string) => {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification('Nexus Alarm', { body: message, icon: '/favicon.svg' });
    }
    // Play sound
